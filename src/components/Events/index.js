...
class Events extends Component {
  state = {activeRegistrationStatus: registrationStatusConstant.initial}

  setActiveRegistrationStatus = registrationStatus => {
  ...
  renderRegistrationDetails = () => {
    ...
    switch (activeRegistrationStatus) {
      case registrationStatusConstant.yetToRegister:
        return <ActiveEventRegistrationDetails status="YET_TO_REGISTER" />
      case registrationStatusConstant.registered:
        return <ActiveEventRegistrationDetails status="REGISTERED" />
      case registrationStatusConstant.registrationsClosed:
        return <ActiveEventRegistrationDetails status="REGISTRATIONS_CLOSED" />
      default:
        return (
  ...
  render() {
    return (
      <div className="events-container">
        <div className="events-item-bg-container">
          ...
          <ul className="ul-container">
            {eventsList.map(event => (
              <EventItem
                eventsList={event}
                key={event.id}
                renderEvent={this.setActiveRegistrationStatus}
              />
            ))}
          </ul>
        </div>
